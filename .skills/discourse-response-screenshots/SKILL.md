...
description: Capture HiDPI screenshots and videos of Discourse UI with a temporary system spec, and include screenshots in the workflow bot's GitHub response. Use when screenshots are requested or help demonstrate a UI change.
---
...
6. Remove the temporary spec and any temporary helpers you created. Do not commit screenshots or capture-only changes; stage the intended product files explicitly. PNGs can remain in ignored `tmp/` until the run ends.
...
For video captures, add `video: true` to the example and wait for the spec to finish before using the resulting `tmp/capybara/*-screenrecord.webm`. Enable animations for demonstrations with `RSpec.configure { |config| config.before(:suite) { Capybara.disable_animation = false } }`. Keep the original resolution when trimming or converting the recording. Video upload is not yet supported by `upload_image`.
